...
class Run:
    ...
    def process(self,hostname,interface,traffic_in,traffic_out,timestamp):
        ...
        if self.already_interface[key]["in"].alert:                                                                                                                  
            with self.lock3:
                self.alerts.append((3,hostname,interface,str(datetime.fromtimestamp(timestamp)),"in",self.transform(list(self.already_interface[key]["in"].data_list_5_days))))
        if self.already_interface[key]["out"].alert:
            with self.lock3:
                self.alerts.append((3,hostname,interface,str(datetime.fromtimestamp(timestamp)),"out",self.transform(list(self.already_interface[key]["out"].data_list_5_days))))
    ...
    def post(self,producer,alert):
        kafka_msg={}
        if alert[0]==1:
            kafka_msg={
                "alert_type":"task_accumulation",
            ...
            kafka_msg={
                ...
                "alert_msg":{
                    "hostname":alert[1],
                    "interface":alert[2],
                    "traffic_direction":alert[4],
                    "alert_time":alert[3],
                    "recent_traffic":alert[-1],
                },
                "create_time":datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            }
        try:
            producer.send(
                topic="traffic_drop_monitor",
                value=kafka_msg
            )
            logging_alerts.info(
                f"Kafka消息发送成功。"
            ...
            logging_alerts.error(
                f"Kafka消息发送失败 | 错误原因: {str(e)} | 消息内容: {kafka_msg}。"
            )
